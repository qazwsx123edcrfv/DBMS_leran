...
def user_login():
    ...
    sql = text('select * from user where telephone = :userortel and password = :password')
    data = db.session.execute(sql, {"userortel": userortel, "password": password}).first()
    print(data)
    if data != None:
...
def user_addorder():
    ...
    db.session.execute(
        text('insert into oorder(shop_name, order_money, order_way, cons_phone, cons_name, cons_addre, create_time) values(:shop_name, :order_money, :order_way, :cons_phone, :cons_name, :cons_addre, :create_time)'),
        {
            'shop_name': shopname,
...
def user_unsend():
    if request.method == 'GET':
        ...
        data = db.session.execute(
            text('select * from oorder where checked=0 and cons_phone=:phone'),
            {"phone": phone}
        ).fetchall()
    ...
    if request.method == 'DELETE':
        ...
        db.session.execute(
            text('delete from oorder where order_id=:order_id'),
            {"order_id": order_id}
        )
        db.session.commit()
    ...
    return jsonify(status=400, msg="请求错误")
    ...
    if request.method == 'GET':
        ...
        for i in range(len(data)):
            dic = dict(order_id=data[i][0], shop_name=data[i][1], order_money=data[i][2], order_way=data[i][3],
                       cons_phone=data[i][4], cons_name=data[i][5], cons_addre=data[i][6], disp_id=data[i][7],
                       deliver_time=data[i][8], disp_phone=data[i][9])
            Data.append(dic)
        return jsonify(status=200, tabledata=Data)
...
def usermsg():
    if request.method == 'GET':
        phone = get_token_phone(request.headers.get('token'))
        data = db.session.execute(text('select * from user_msg where phone=:phone'), {"phone": phone}).fetchall()
        ...
        if data:
            ...
            return jsonify(status=404, msg="User data not found")
...
def user_pwd_chg():
    if request.method == 'POST':
        ...
        if request.method == 'POST':
            ...
            old_pwd = request.json.get('old_pwd')
            phone = get_token_phone(request.headers.get('token'))
            sql_update = text('update user set password=:pwd where telephone=:phone')
            db.session.execute(sql_update, {"pwd": pwd, "phone": phone})
            db.session.commit()
            return jsonify(status=200, msg="修改成功")
...
def manager_shop():
    ...
    if request.method == 'POST' and request.json.get('action') == "add":
        ...
        sql_select = text('select * from fastfood_shop where shop_name=:shop_name')
        exist = db.session.execute(sql_select, {"shop_name": shop_name}).fetchall()

        if not exist:
            sql_insert = text(
                'insert into fastfood_shop (shop_name, price, m_sale_v) values (:shop_name, :price, :m_sale_v)')
            db.session.execute(sql_insert, {"shop_name": shop_name, "price": int(price), "m_sale_v": int(m_sale_v)})
            db.session.commit()
    ...
    def manager_shop():
        if request.method == 'POST' and request.json.get('action') == "change":
            ...
            price = rq.get('price')
            m_sale_v = rq.get('m_sale_v')
            sql_update = text('update fastfood_shop set price=:price, m_sale_v=:m_sale_v where shop_name=:shop_name')
            db.session.execute(sql_update, {"price": int(price), "m_sale_v": int(m_sale_v), "shop_name": shop_name})
            db.session.commit()
            return jsonify(status=200, msg="修改成功")

        if request.method == 'DELETE':
            want_delete = request.json.get('want_delete')
            sql_delete = text('delete from fastfood_shop where shop_name=:want_delete')
            db.session.execute(sql_delete, {"want_delete": want_delete})
            db.session.commit()
            return jsonify(status=200, msg="删除成功")
...
def manager_server():
    ...
    if request.method == 'POST':
        ...
        exist = db.session.execute(text('select * from server where service_id=:service_id'), {'service_id': service_id}).fetchall()
        if not exist:
            db.session.execute(
                text('insert server(service_id,service_name,fastfood_shop_name) value(:service_id,:service_name,:fastfood_shop_name)'),
                {'service_id': service_id, 'service_name': service_name, 'fastfood_shop_name': fastfood_shop_name}
            )
            db.session.commit()
            return jsonify(status=200, msg="添加成功")
        else:
    ...
    if request.method == 'DELETE':
        want_delete = request.json.get('want_delete')
        db.session.execute(text('delete from server where service_id=:want_delete'), {'want_delete': want_delete})
        db.session.commit()
...
def manager_dispatcher():
    ...
    if request.method == 'POST':
        ...
        exist = db.session.execute(text('select * from dispatcher where dispatcher_id=:dispatcher_id'), {'dispatcher_id': dispatcher_id}).fetchall()
        if not exist:
            db.session.execute(
                text('insert dispatcher(dispatcher_id,dispatcher_name,dispatcher_phone) value(:dispatcher_id,:dispatcher_name,:dispatcher_phone)'),
                {'dispatcher_id': dispatcher_id, 'dispatcher_name': dispatcher_name, 'dispatcher_phone': dispatcher_phone}
            )
            db.session.commit()
            return jsonify(status=200, msg="添加成功")
        else:
    ...
    if request.method == 'DELETE':
        want_delete = request.json.get('want_delete')
        db.session.execute(text('delete from dispatcher where dispatcher_id=:want_delete'), {'want_delete': want_delete})
        db.session.commit()
        return jsonify(status=200, msg="解雇成功")
...
def manager_unsend():
    if request.method == 'GET':
        ...
        disp_range = db.session.execute(text('select * from dispatcher')).fetchall()
        Disp_range = []
    ...
    if request.method == 'POST':
        ...
        db.session.execute(
            text('insert into wuliu (order_id, cons_phone, disp_id, deliver_time) values (:order_id, :cons_phone, :disp_id, :deliver_time)'),
            {"order_id": int(order_id), "cons_phone": cons_phone, "disp_id": disp_id, "deliver_time": deliver_time}
        )
        db.session.commit()
